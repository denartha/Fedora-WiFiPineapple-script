#!/bin/bash
# Setup Internet Connection Sharing on Fedora 18+
#define variables
...
echo -n "Interface between PC and Pineapple [em1]: "
read pineapplelan
if [[ $pineapplelan == '' ]]; then 
pineapplelan=em1 # Interface of ethernet cable directly connected to Pineapple
fi

echo -n "Interface between PC and Internet [wlan0]: "
read pineapplewan
if [[ $pineapplewan == '' ]]; then 
pineapplewan=wlan0 #i.e. wlan0 for wifi, ppp0 for 3g modem/dialup, em1 for lan
fi
...
#setup IP forwarding
iptables -A FORWARD -i $pineapplewan -o $pineapplelan -s $pineapplenet -m conntrack --ctstate NEW -j ACCEPT
